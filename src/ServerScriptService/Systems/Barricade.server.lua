...
local function isValidPlacement(anchor)
  ...
  -- Check for overlapping parts using OverlapParams (modern API)
  local overlapParams = OverlapParams.new()
  overlapParams.FilterType = Enum.RaycastFilterType.Exclude
  overlapParams.FilterDescendantsInstances = { anchor }
  overlapParams.MaxParts = 100

  local size = Vector3.new(4, 4, 1) -- 2 studs in each direction from center
  local parts = workspace:GetPartBoundsInBox(anchor.CFrame, size, overlapParams)
