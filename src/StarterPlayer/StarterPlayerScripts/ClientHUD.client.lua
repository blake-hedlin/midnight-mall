...
-- Responsive scaling: bound to viewport (per ux-context mobile-safe requirements)
local Camera = workspace.CurrentCamera
local function getViewportScale()
  local viewportSize = Camera.ViewportSize
  local minDimension = math.min(viewportSize.X, viewportSize.Y)
  -- Base scale on 1080p reference; scale down for smaller viewports
  return math.clamp(minDimension / 1080, 0.5, 1.2)
end
...
-- Update scale on viewport changes
...
-- Function to update time frame size based on viewport
local function updateTimeFrameSize()
  local viewportSize = Camera.ViewportSize
  if viewportSize.X < 720 then
    -- Mobile/narrow: use proportional width (35% of screen minus margins)
    timeFrame.Size = UDim2.new(0.35, -HUD_SAFE_MARGIN, 0, 40)
  else
    -- Desktop: fixed width
...
updateTimeFrameSize()
Camera:GetPropertyChangedSignal("ViewportSize"):Connect(updateTimeFrameSize)
...
-- Responsive sizing: proportional width on narrow viewports (mobile)
...
-- Function to update inventory frame size based on viewport
local function updateInventorySize()
  local viewportSize = Camera.ViewportSize
  if viewportSize.X < 720 then
    -- Mobile/narrow: use proportional width (30% of screen minus margins)
    invFrame.Size = UDim2.new(0.3, -HUD_SAFE_MARGIN, 0, 105)
  else
    -- Desktop: fixed width
...
updateInventorySize()
Camera:GetPropertyChangedSignal("ViewportSize"):Connect(updateInventorySize)
...
local function updateInventoryDisplay(itemName)
  -- Update specific item or all items with O(1) lookup and rich animation
  ...
  for _, name in ipairs(itemsToUpdate) do
    ...
    local config = itemConfigLookup[name] -- O(1) lookup from main branch

    if label and config then
      -- Update text
      label.Text = config.emoji .. " " .. config.label .. ": " .. count

      -- Rich animation: scale + color pulse (from story7 branch, using FS_MD from main)
      local tweenInfo = TweenInfo.new(TWEEN_FEEDBACK, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
      local scaleUp = TweenService:Create(label, tweenInfo, { TextSize = FS_MD * 1.2 })
      local scaleDown = TweenService:Create(label, tweenInfo, { TextSize = FS_MD })
...
Signals.StateTick.OnClientEvent:Connect(function(state, time, nightCount)
  if state == "Day" then
    timeLabel.Text = ("Day %d — %ds left"):format(
      nightCount + 1,
      math.max(0, 120 - math.floor(time))
    )
    timeFrame.BackgroundColor3 = Color3.fromRGB(100, 150, 255) -- Blue for day
  else
    timeLabel.Text = ("Night %d — %ds left"):format(
      nightCount,
      math.max(0, 60 - math.floor(time))
    )
